...
public class BehaviorTests
{
    ...
    [Fact]
    public async Task LoggingBehavior_Should_Log_Errors_With_Redacted_Message()
    {
        // Arrange
        var logger = new FakeMediatorLogger();
        var behavior = new Behaviors.LoggingBehavior<LoggedRequest, string>(logger); // Default options
        var request = new LoggedRequest("fail");
        ...
        // Assert that the exception message is redacted by default
        var loggedException = logger.ErrorMessages[0].Exception;
        Assert.IsType<InvalidOperationException>(loggedException);
