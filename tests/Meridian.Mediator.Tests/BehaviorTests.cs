...
public class BehaviorTests
{
    ...
    public async Task LoggingBehavior_Should_Log_Errors_With_Redacted_Message_By_Default()
    {
        ...
        // Assert that the exception was sanitized to hide the actual message
    ...
    [Fact]
    public async Task LoggingBehavior_Should_Log_Errors_With_Original_Message_When_Configured()
    {
        // Arrange
        var logger = new FakeMediatorLogger();
        ...
        // Assert that the exception was sanitized but original message was preserved
        var loggedException = logger.ErrorMessages[0].Exception;
        Assert.IsType<InvalidOperationException>(loggedException);
