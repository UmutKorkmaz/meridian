...
public class BehaviorTests
{
    ...
    [Fact]
    public async Task LoggingBehavior_Should_Log_Errors_SanitizesMessageByDefault()
    {
        // Arrange
        var logger = new FakeMediatorLogger();
        var behavior = new Behaviors.LoggingBehavior<LoggedRequest, string>(logger);
    ...
    [Fact]
    public async Task LoggingBehavior_Should_Log_Errors_PreservesMessageWhenTelemetryOptionTrue()
    {
        // Arrange
        var logger = new FakeMediatorLogger();
        ...
        // Assert that the exception message is preserved
        var loggedException = logger.ErrorMessages[0].Exception;
        Assert.IsType<InvalidOperationException>(loggedException);
