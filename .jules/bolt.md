...
## 2026-06-25 - Avoid LINQ enumeration in NotificationPublishers
**Learning:** In Notification Publishers (like ForeachAwaitPublisher and TaskWhenAllPublisher), iterating over `IEnumerable<T>` natively allocates enumerators. By using fast paths for arrays and `IList<T>`, we can avoid these allocations via indexing `for` loops.
**Action:** When working with collections of `NotificationHandlerExecutor` or handlers passed into publishers, bypass `foreach` and `IEnumerable` operations by adding fast paths checking for `object[] { Length: 0 }`, `T[]`, and `IList<T>` arrays to loop sequentially via index, which has measurable impact.
