...
## 2024-07-01 - Avoid Per-Request Memory Allocations in Pipeline Behaviors
**Learning:** Behaviors that loop over lists and track errors (like validation or resilient publishing) were eagerly instantiating `new List<Exception>()` or `new List<ValidationError>()` even when no errors occurred, contributing to hidden baseline memory allocations. Additionally, bypassing loops entirely for empty DI-injected pipelines requires a fast-path check, but duplicating the loop logic for `IList` iteration degrades maintainability.
**Action:** When tracking issues within pipelines, use lazy allocation (e.g. `errors ??= new List<ValidationError>();`) to keep the "happy path" allocation-free. For empty pipeline scenarios, implement a fast-path check using `is ICollection<T> { Count: 0 }` (which handles DI-injected arrays safely) before defaulting to a standard `foreach` loop, preserving both performance and readability.
