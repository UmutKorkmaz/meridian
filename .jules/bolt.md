...
## 2024-06-28 - Zero-allocation enumeration via IList/IReadOnlyList for-loop in ForeachAwaitPublisher and Pre/Post behaviors
**Learning:** `foreach` loops on `IEnumerable<T>` allocate an enumerator object on the heap per iteration. When iterating over arrays or lists, we can optimize this via an explicit interface type-check (like `IList` or `IReadOnlyList`) followed by a standard `for` loop, eliminating the heap allocation completely. A fast check like `object[] { Length: 0 }` offers additional early exit speed due to C# array covariance.
**Action:** When iterating over collections returned via DI (which are mostly arrays), always apply a fast-path zero-allocation loop using interface matching (e.g. `is IList<T>` or `is IReadOnlyList<T>`) before falling back to `foreach`. Use `is object[] { Length: 0 }` or `is ICollection<T> { Count: 0 }` for fast early returns.
